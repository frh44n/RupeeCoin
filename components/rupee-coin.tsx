  ...
  return (
    <div className="relative flex items-center justify-center w-64 h-64">
      {fallingCoins.map((coin) => (
        ...
        >
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-yellow-400 via-yellow-500 to-yellow-600 border-2 border-yellow-300 shadow-lg flex items-center justify-center">
      ...
      {floatingTexts.map((text) => (
        <div
          key={text.id}
          className="absolute pointer-events-none animate-float-up text-3xl font-bold text-green-400 drop-shadow-lg"
          style={{
            left: `50%`,
            top: `50%`,
      ...
      {tapEffect && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="w-32 h-32 border-4 border-yellow-400/60 rounded-full animate-ping-3d" />
          <div className="absolute w-40 h-40 border-2 border-yellow-300/40 rounded-full animate-ping-3d animation-delay-100" />
          <div className="absolute w-48 h-48 border border-yellow-200/20 rounded-full animate-ping-3d animation-delay-200" />
        </div>
      )}
      ...
      <button
        ...
        className={`
          ...
          ${tapEffect ? "scale-110 rotate-y-12" : ""}
        `}
        style={{
          background: `
      ...
      >
        <div className="absolute inset-3 rounded-full bg-gradient-to-br from-yellow-200/60 via-transparent to-yellow-800/20" />

        <div className="absolute top-6 left-8 w-12 h-12 bg-white/50 rounded-full blur-md" />
        <div className="absolute top-12 left-12 w-6 h-6 bg-white/70 rounded-full blur-sm" />
        <div className="absolute bottom-8 right-10 w-8 h-8 bg-yellow-800/30 rounded-full blur-sm" />

        <div className="absolute inset-0 flex items-center justify-center transform-style-3d">
          <div
            className="text-7xl font-black text-yellow-900 drop-shadow-2xl transform transition-transform duration-200"
            style={{
              textShadow: `
